  ...
  Object.entries({
    ...
    approvalMode: {
      ...
      gemini: {
        flag: "--approval-mode",
        value: "enum",
        allowed: ["default", "auto_edit", "yolo"],
      },
      "cursor-agent": null,
    },
    worktree: {
...
function extractFlags(tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (
      t.startsWith("--") ||
      (t.startsWith("-") && t.length === 2 && !t.match(/^-\d/))
    ) {
      if (t.includes("=")) {
        const eq = t.indexOf("=");
...
function transformParsedFlags(parsedFlags, sourceCli, targetCli) {
  ...
  for (const { flag, value } of parsedFlags) {
    ...
    if (!concept) {
      result.passthrough.push({
        flag,
        value,
        reason: "unknown flag, passed through",
      });
      continue;
    }
    ...
    if (!targetSpec) {
      result.dropped.push({
        ...
        value,
        concept,
        reason: `${targetCli} has no equivalent for '${concept}'`,
      });
      continue;
    }
...
export function transformFlags(argv, targetCli, sourceCli) {
  if (!CLIS.includes(targetCli)) {
    throw new Error(
      `Unknown target CLI: ${targetCli}. Known: ${CLIS.join(", ")}`,
    );
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (targetIdx === -1) {
    console.error(
      "Usage: transform-flags.mjs --target <cli> [--source <cli>] <flags...>",
    );
    console.error(
      "  --target, -t   Target CLI format (claude|codex|gemini|cursor-agent)",
    );
    console.error(
      "  --source       Source CLI format (auto-detected if omitted)",
    );
    console.error("");
    console.error("Example:");
    console.error(
      "  node transform-flags.mjs --target codex --model claude-3-opus --yolo --verbose",
    );
    process.exit(1);
  }
  ...
  const flagArgs = args.filter((_, i) => {
    ...
    if (sourceIdx !== -1 && (i === sourceIdx || i === sourceIdx + 1))
      return false;
    return true;
  });
