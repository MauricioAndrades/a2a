...
const mcp = new Server(
    { name: "a2a-channel", version: "1.0.0" },
    {
        ...
        instructions: [
            ...
            "Sending between agents: POST /api/a2a/send JSON { to, from, origin, body, action } with origin user|peer|self. Local delivery: wrap body in <a2a_message from to origin ts>…</a2a_message> with XML-escaped body text, then tmux paste-buffer into tmuxTarget and press Enter. If recipient has bridgeUrl, the bridge forwards the same payload to that URL's /api/a2a/send instead.",
