...
test("flag-form rejects unknown bare flags against registry", () => {
  assert.throws(
    () => parseFlagSendArgv(["--verbose", "hello"], registry),
...
test("flag-form folds --to into normalized recipients", () => {
  assert.deepEqual(
    parseFlagSendArgv(["--to", "bob", "--leah", "hello"], registry),
    {
      action: "message",
      recipients: ["bob", "leah"],
      broadcast: false,
...
test("flag-form accepts glob recipient selector flags", () => {
  assert.deepEqual(
    parseFlagSendArgv(["--write", "--*managers", "hello"], registry),
    {
      action: "message",
      recipients: ["*managers"],
      broadcast: false,
...
test("flag-form accepts glob values for --to", () => {
  assert.deepEqual(
    parseFlagSendArgv(["--ask", "--to", "*managers", "status"], registry),
    {
      action: "ask",
      recipients: ["*managers"],
      broadcast: false,
...
test("flag-form validates recipients when only agents OR groups Set is present", () => {
  assert.deepEqual(
    parseFlagSendArgv(["--reply", "--bob", "hey"], {
      agents: new Set(["bob"]),
    }),
    {
      action: "reply",
  ...
  assert.throws(
    () =>
      parseFlagSendArgv(["--reply", "--eve", "no"], {
        agents: new Set(["bob"]),
      }),
    /unknown flag --eve/,
  );
...
test("colon-form parses action, recipients, sender, origin, and repeated recipients", () => {
  assert.deepEqual(
    parseColonFlagArgv(
      ["--ask:bob:leah:bob", "--from=op", "--origin=user", "status"],
      registry,
    ),
    {
      from: "op",
...
test("colon-form keeps unknown equals flags as metadata", () => {
  assert.deepEqual(
    parseColonFlagArgv(["--message:bob", "--mood=angry", "status"], registry),
    {
      from: null,
      origin: null,
      recipients: ["bob"],
...
test("colon-form equals value is message content", () => {
  assert.equal(
    parseColonFlagArgv(["--message:bob=value"], registry).content,
    "value",
  );
});
...
test("colon-form rejects duplicate inline and positional content", () => {
  assert.throws(
    () => parseColonFlagArgv(["--message:bob=value", "other"], registry),
    /message content specified more than once/,
  );
});
