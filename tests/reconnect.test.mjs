...
test("explicit name resolves to that target", () => {
  assert.deepEqual(
    resolveReconnectTargets({ ...base, name: "bob", hasAll: false }),
    { targets: ["bob"], viewSession: null },
  );
});

test("--all resolves all live non-view sessions", () => {
  assert.deepEqual(
    resolveReconnectTargets({ ...base, name: null, hasAll: true }),
    { targets: ["bob", "leah"], viewSession: "a2a-view" },
  );
});

test("no name prefers cached live agents", () => {
  assert.deepEqual(
    resolveReconnectTargets({ ...base, name: null, hasAll: false }),
    { targets: ["leah"], viewSession: null },
  );
});

test("corrupt registry agents skips cache slice without guessing live sessions", () => {
  assert.deepEqual(
